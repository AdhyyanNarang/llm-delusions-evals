...
from llm_delusions_evals.constants import format_model_label, normalize_model_label
...
def _summary_score_stats(info: dict[str, Any]) -> dict[str, float]:
    ...
    return {
        ...
        "ci_lower": float(info.get("ci_lower", float("nan"))),
        "ci_upper": float(info.get("ci_upper", float("nan"))),
        "samples": float(info.get("samples", 0)),
    }
...
def fig_main_heatmap(summary: dict) -> None:
    ...
    pivot = category_df.pivot(index="model_label", columns="group", values="mean")
...
def fig_code_heatmap(summary: dict, *, include_validates_codes: bool) -> None:
    ...
    pivot = df.pivot(index="model_label", columns="code_short", values="mean")
...
def fig_temporal(cat_df: pd.DataFrame, *, include_validates_codes: bool) -> None:
    ...
    for axis_index, (panel_title, panel_variants) in enumerate(panel_specs):
        ...
        panel_models, missing_display_names = _resolve_panel_models(
            cat_df,
            panel_variants,
        )
        ...
        _draw_reasoning_panel(ax, panel_df, available_groups, panel_models)
    ...
    fig.subplots_adjust(bottom=0.33, wspace=0.08, hspace=0.30)
...
def _resolve_panel_models(
    ...
    return panel_models, missing_display_names
...
def _export_category_summary_tables(summary: dict) -> None:
    ...
    cat_pivot = df_cat.pivot(index="model_label", columns="group", values="prevalence")
...
def _export_code_summary_tables(
    ...
    *,
    ...
    if code_rows:
        ...
        code_pivot = df_code.pivot(
            index="code_short", columns="model_label", values="prevalence"
        )
        model_cols = sort_model_labels(list(code_pivot.columns))
        code_pivot = code_pivot[model_cols]
        code_pivot["category"] = code_pivot.index.map(
            lambda c: CODE_CATEGORIES.get(c, "unknown")
        )
        if not include_validates_codes:
            code_pivot = code_pivot[
                ~code_pivot.index.to_series().map(_is_validates_code)
            ]
        if code_pivot.empty:
            logger.warning(
            ...
            code_order_map = {
                code: index for index, code in enumerate(_CODE_ORDER_PREF)
            }
            code_pivot = code_pivot.sort_values(
                by="code_short",
                key=lambda col: col.map(lambda val: code_order_map.get(val, 999)),
            )
            code_pivot = code_pivot.reset_index()
            code_pivot = code_pivot[["code_short", "category"] + model_cols]

            csv_path = _save_csv(code_pivot, "prevalence_by_model_code.csv")
            tables_dir = TABLE_DIR
            _csv_to_latex_tabular(
                csv_path,
                tables_dir / "prevalence_by_model_code.tex",
                config=LaTeXTableConfig(
                    col_spec="ll" + "r" * len(model_cols),
                    header_labels={"code_short": "Code", "category": "Category"},
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Include validates-* codes in code-level figures/tables. "
            "By default these codes are excluded from paper-export plots."
        ),
    )
    parser.add_argument(
        "-v",
    ...
    fig_temporal(cat_df, include_validates_codes=args.include_validates_codes)
