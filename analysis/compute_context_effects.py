...
from llm_delusions_evals.constants import format_model_label, normalize_id
from llm_delusions_evals.participant_exclusions import resolve_excluded_window_ids
from llm_delusions_evals.paths import DEFAULT_WINDOWS_PATH, resolve_path
...
def _aggregate_grouped_context_effects(
    ...
    for group_key, group_df in rows_df.groupby(group_cols, dropna=False):
        window_ids = (
            group_df["window_id"].astype(str).str.strip()
    ...
    *,
    ...
    for cohort_key, cohort_df in capped.groupby(cohort_cols, dropna=False):
        ...
        for requested in requested_values_for_intersection:
            ...
            window_ids = set(requested_df["window_id"].astype(str))
            window_ids.discard("")
        ...
        retained_parts.append(
            cohort_df[cohort_df["window_id"].astype(str).isin(shared_window_ids)]
        )
        logger.info(
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "LLM_DELUSIONS_EXCLUDED_PARTICIPANTS (default: 103,107,115). "
...
def _write_standard_outputs(
    ...
    *,
    ...
    logger.info(
        ...
        "%d appendix subplot figure(s), and two CSVs in %s",
        len(figure_paths),
        figures_dir,
...
def _regenerate_standard_figures_only(
    *,
    ...
    logger.info(
        ...
        "and %d appendix subplot figure(s) in %s.",
        len(figure_paths),
        len(category_figure_paths),
        appendix_plot_count,
        figures_dir,
