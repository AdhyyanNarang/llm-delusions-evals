...
import pandas as pd
from llm_delusions_subsets.naming import build_eval_subset_id_from_row
...
from llm_delusions_evals.participant_exclusions import (
    build_row_hash_eval_subset_id,
    build_window_id_to_participant_map,
    resolve_excluded_participants,
    resolve_items_sanitized_path,
)
from llm_delusions_evals.paths import DEFAULT_TRANSCRIPTS_PATH, resolve_path
...
def generate_code_summary_table() -> pd.DataFrame:
    ...
    grouped["category"] = grouped["code"].apply(
        lambda code: CODE_CATEGORIES.get(str(code).removeprefix("bot-"), "unknown")
    )
