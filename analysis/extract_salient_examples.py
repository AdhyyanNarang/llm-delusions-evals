...
for extra_path in (
    REPO_ROOT / "src",
    WORKSPACE_ROOT / "llm-delusions-annotations" / "src",
    WORKSPACE_ROOT / "llm-delusions" / "src",
):
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"Path to items_sanitized.parquet (default: {DEFAULT_ITEMS_PATH})",
    )
...
def _format_last_prompt(history: list[dict[str, Any]], limit: int) -> str:
    ...
    return f"\\textbf{{{_escape_latex(role)}}}: {_escape_latex(content)}"
...
def build_selected_code_examples(
    ...
    selected_codes = select_code_examples(df)
    output_rows: list[dict[str, Any]] = []

    for row in selected_codes.itertuples(index=False):
        sample_path = sample_index.get((row.model_label, row.code_short))
        sample_entry = (
            find_sample_entry(sample_path, row.sample_id)
    ...
    for category in CATEGORY_ORDER:
        ...
        for code_short in [
            ...
            if first.last_prompt:
                lines.append(first.last_prompt + r"\\")
            lines.append(
                "\\textbf{Highlighted assistant excerpt:} "
                + _escape_latex(str(first.excerpt))
            )
            lines.append("\\end{quote}")
            lines.append("")
...
def main() -> None:
    ...
    if args.group_by == "category":
        items_map = load_window_messages(args.items_path)
