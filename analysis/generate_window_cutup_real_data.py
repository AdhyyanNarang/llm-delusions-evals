...
class FigureTemplatePayload:
    ...
    Parameters
    ----------
    original_match_pairs:
        Two original-output match lists for ``A1`` and ``A2``.
    evaluated_match_pairs:
        Two evaluated-output match strings for ``A'_1`` and ``A'_2``.
    behavior_labels:
        Four behavior labels for ``A1``, ``A2``, ``A'_1``, and ``A'_2``.
    """

    original_match_pairs: tuple[list[str], list[str]]
    evaluated_match_pairs: tuple[str, str]
...
def render_tikz_figure_body(
    ...
    """
    original_a1_matches, original_a2_matches = payload.original_match_pairs
    evaluated_a1_match, evaluated_a2_match = payload.evaluated_match_pairs
    (
    ...
    replacements = {
        ...
        "__ORIGINAL_A1_MATCH__": escape_latex(
            truncate_text(
                (
                    normalize_text(" ... ".join(original_a1_matches))
                    if original_a1_matches
                    else "..."
                ),
                220,
            )
        ),
        "__ORIGINAL_A2_MATCH__": escape_latex(
            truncate_text(
                (
                    normalize_text(" ... ".join(original_a2_matches))
                    if original_a2_matches
                    else "..."
                ),
                220,
            )
        ),
        "__EVALUATED_A1_MATCH__": escape_latex(evaluated_a1_match),
        "__EVALUATED_A2_MATCH__": escape_latex(evaluated_a2_match),
...
def main() -> None:
    ...
    original_first_matches = extract_original_item_matches(
        item_row=item_row,
        code=args.code,
        turn_index=first_turn.turn_index,
    )
    original_second_matches = extract_original_item_matches(
        item_row=item_row,
        code=args.code,
        turn_index=second_turn.turn_index,
    )
    evaluated_first_match = extract_match_text(
        first_turn,
        use_original=False,
        max_chars=180,
    )
    evaluated_second_match = extract_match_text(
        second_turn,
        use_original=False,
        max_chars=180,
    )
    ...
    tikz_body = render_tikz_figure_body(
        ...
        payload=FigureTemplatePayload(
            original_match_pairs=(original_first_matches, original_second_matches),
            evaluated_match_pairs=(evaluated_first_match, evaluated_second_match),
