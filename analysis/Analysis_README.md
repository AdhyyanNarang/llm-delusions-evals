...
By default, report/context analyses exclude participants `103,107,115`.
This behavior is shared across scripts and controlled by
`LLM_DELUSIONS_EXCLUDED_PARTICIPANTS`.

```bash
# Exclude participants 103,107,115 (same as default behavior)
LLM_DELUSIONS_EXCLUDED_PARTICIPANTS=103,107,115 \
uv run python -m llm_delusions_evals.scripts.generate_report \
    --logs-dir logs --output-dir report
...
The same env var applies to context scripts:

```bash
# Context effects with exclusions
LLM_DELUSIONS_EXCLUDED_PARTICIPANTS=103,107,115 \
uv run python -m analysis.compute_context_effects --logs-dir logs-context

# Context effects with all participants
LLM_DELUSIONS_EXCLUDED_PARTICIPANTS="" \
uv run python -m analysis.compute_context_effects --logs-dir logs-context

# Context controls with exclusions
LLM_DELUSIONS_EXCLUDED_PARTICIPANTS=103,107,115 \
uv run python -m analysis.compute_context_code_controls --logs-dir logs-context

# Context controls with all participants
LLM_DELUSIONS_EXCLUDED_PARTICIPANTS="" \
uv run python -m analysis.compute_context_code_controls --logs-dir logs-context
```
...
Use `--excluded-participants ""` to disable exclusions.

For dataset-level methods stats, optionally override the sanitized parquet path:
...
### Sensitive Data Policy (Context Effects)

Reminder: remove context logs, row caches, and derived context-effect outputs
before any public push or release branch.
...
Overleaf export is decoupled from artifact generation.
After running producer scripts, explicitly export selected assets:

```bash
uv run python -m analysis.export_overleaf_assets
```

The export selection is controlled by
`analysis/paper_export_manifest.json`. The manifest is organized by
producer (`generate_figures`, `generate_window_cutup_real_data`,
`compute_context_effects`, `compute_context_code_controls`) so exported files
are an explicit, audited subset of produced outputs.
