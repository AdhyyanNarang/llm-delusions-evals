...
from llm_delusions_evals.constants import get_source_id


def _get_cutoffs_mapping() -> dict[str, int]:
    """Load the cutoffs mapping from the annotations package or fallback path."""
    try:
    ...
    # Fallback to sibling repository location if package data is missing/stale
    repo_root = Path(__file__).resolve().parent.parent.parent.parent.parent
    fallback_path = (
        repo_root
        / "llm-delusions"
        / "src"
        / "llm_delusions_annotations"
        / "data"
        / "cutoffs.csv"
    )
    if fallback_path.is_file():
        return load_cutoffs_mapping(str(fallback_path))

    logging.warning("Could not find cutoffs.csv in package or fallback path.")
    return {}
