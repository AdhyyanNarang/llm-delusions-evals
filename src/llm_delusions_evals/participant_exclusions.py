...
3. How eval ``window_id`` values map back to participant IDs.
...
import pandas as pd
from llm_delusions_subsets.naming import build_eval_subset_id_from_row, build_window_id

from llm_delusions_evals.paths import _DATA_REPO_ROOT, resolve_path

EXCLUDED_PARTICIPANTS_ENV = "LLM_DELUSIONS_EXCLUDED_PARTICIPANTS"
DEFAULT_EXCLUDED_PARTICIPANTS = ("103", "107", "115")

ITEMS_SANITIZED_PATH_ENV = "LLM_DELUSIONS_ITEMS_SANITIZED_PATH"
DEFAULT_ITEMS_SANITIZED_PATH = str(
    _DATA_REPO_ROOT / "subsets" / "items_sanitized.parquet"
)
...
def resolve_excluded_participants(raw_value: str | None = None) -> set[str]:
    ...
    raw_value:
        ...
        If both are unset, default exclusions are returned.
...
def build_row_hash_eval_subset_id(row_index: int) -> str:
    """Return the fallback eval subset id used for sanitized export rows.
...
def build_window_id_to_participant_map(windows_path: str | Path) -> dict[str, str]:
    ...
    items_path = windows_path.with_name("items.parquet")
    ...
    for row_index, row in enumerate(selected_items.itertuples()):
        ...
        if not participant:
            continue
        mapping[build_row_hash_eval_subset_id(row_index)] = participant
