...
This is an eval designed to measure whether models encourage users' delusions.

## Release Notes

- This repository is intended for a one-time release; external contributions are not accepted.
- TODO: add a concise data and access policy summary before public publication.

### Large Artifact Storage (GCS)

Large generated artifacts are stored outside Git, following the same pattern used
in `../llm-delusions` (using `gcloud storage rsync`).
...
```sh
# Upload large artifacts to GCS
bash scripts/sync_artifacts_gcs.sh upload <bucket> [prefix]

# Download them back locally
bash scripts/sync_artifacts_gcs.sh download <bucket> [prefix]
```

You can also set defaults through environment variables:

```sh
export LLM_DELUSIONS_EVALS_ARTIFACT_BUCKET=<bucket>
export LLM_DELUSIONS_EVALS_ARTIFACT_PREFIX=<prefix>
```

### One-Time Public Copy

Create a public-ready filesystem copy from tracked files:
...
`/private/tmp/llm-delusions-evals-public`

Default exclude list:

`scripts/public_export_excludes.txt`
...
Inspect AI natively supports models with reasoning capabilities (e.g., OpenAI `o1`/`o3-mini`, DeepSeek `r1`, Anthropic Claude 3.7). You can modulate reasoning behavior using the following CLI flags:
...
Alternatively, these can be set directly in Python code within the `eval()` function or `GenerateConfig` using arguments like `reasoning_tokens=0` or `reasoning_effort="none"`.
...
**Google (Vertex AI / GenAI) Safety Settings**
Google models enforce safety settings that might block generations, especially during red-teaming/delusion evals. **Inspect AI automatically defaults to turning all safety categories to `none` (allow all) for all Google models.** This applies to both the evaluated model (`--model`) and the grader model (`--model-role grader=...`). You do not need to manually pass a safety settings configuration to avoid blocks.

If you ever need to pass specific configurations _only_ to the grader model (for example, if you wanted to test different settings), you can pass a JSON string directly to the `--model-role` flag:
...
First, `git clone` [jlcmoore/llm-delusions](https://github.com/jlcmoore/llm-delusions) locally. Then generate the conversation window subsets (see the llm-delusions README for details).

By default, the evals assume that the `llm-delusions` repository is cloned in a sibling directory to `llm-delusions-evals`. If your data is located elsewhere, you can provide these optional environment variable overrides:

```sh
export LLM_DELUSIONS_WINDOWS_PATH=../llm-delusions/subsets/items_sanitized.parquet
```

Optional analysis/report overrides:

```sh
# Default participant exclusions are 103,107,115.
# Set empty string to include all participants in report/context analyses.
export LLM_DELUSIONS_EXCLUDED_PARTICIPANTS=103,107,115

# Optional: point methods stats to an alternate sanitized parquet.
export LLM_DELUSIONS_ITEMS_SANITIZED_PATH=../llm-delusions/subsets/items_sanitized_no_103_107_115.parquet
```

For the Context + Window eval you need transcripts and the non-sanitized
windows parquet:
...
| :--------------------- | :----------- | :------ | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `max_context_messages` | `int`        | `0`     | Number of preceding transcript messages to prepend to the window. If `0`, evaluates only the conversation window. If the history exceeds this limit, only the most recent messages (those immediately preceding the window) are kept, and a "Truncated context..." warning will be displayed. |
...
| `classification_error_as_sample_error` | `bool`/`str` | `True` | If `True`, grader parse failures (`ClassificationError`) are raised as sample-level errors instead of being recorded as `NaN` scores. Useful when you want `inspect eval-retry` to pick these up automatically. |
...
```sh
uv run python src/llm_delusions_evals/scripts/inspect_mocked.py eval src/llm_delusions_evals/tasks/delusions_eval.py \
  --model mockllm/model \
...
**Run a quick test on a single behavior (window-only):**
...
**Run multiple specific codes (window-only):**
...
**Run all behaviors on the full sanitized dataset (window-only):**
...
For any given conversation window (typically ~20 messages long), the task generates **one sample per user turn** within that window.

- If a window contains 10 user messages, it will result in 10 unique data points.
- Each data point includes the conversation history up to that specific user turn, so the model under evaluation is asked to generate the next assistant response.
- **Adding context (`max_context_messages > 0`) does not change the number of samples.** It only prepends additional preceding messages to the history of each existing sample, enabling a direct "apples-to-apples" comparison of how context influences the model's response to the exact same user prompts.
...
All of these models have been smoke tested and work.

Copy one pair into your script at a time.
...
Requested scale comparison set (smoke tested on May 1, 2026):
...
Maybe (not on the critical path):
...
uv run python src/llm_delusions_evals/scripts/inspect_mocked.py eval \
  src/llm_delusions_evals/tasks/delusions_eval.py \
  --model "$MODEL" \
  "${REASONING_ARGS[@]}" \
...
After running all of the above models through the experiment, choose one which performs poorly (or variably) and then run it through with different context windows.
...
```sh
uv run python src/llm_delusions_evals/scripts/generate_report.py
```
...
### One-Command Artifact Refresh (with participant exclusions)

Use this script to regenerate the main report, analysis figures/tables,
context-effect outputs, methods stats, salient qualitative examples, and
optionally export selected assets to the sibling Overleaf repo.

This script does **not** rerun model evals; it only rebuilds artifacts from
existing logs.
...
- `LLM_DELUSIONS_EXCLUDED_PARTICIPANTS`:
  defaults to `103,107,115`.
- `LLM_DELUSIONS_ITEMS_SANITIZED_PATH`:
  optional override for `analysis.compute_methods_stats` dataset path
  (for example, a participant-filtered parquet).
- `NO_CACHE_CONTEXT`:
  defaults to `1`, which passes `--no-cache` to context analysis scripts.
- `EXPORT_OVERLEAF_ASSETS`:
  set to `1` to run `analysis.export_overleaf_assets` at the end.

Example using a participant-filtered sanitized parquet and exporting to
Overleaf:

```sh
LLM_DELUSIONS_ITEMS_SANITIZED_PATH=../llm-delusions/subsets/items_sanitized_no_103_107_115.parquet \
EXPORT_OVERLEAF_ASSETS=1 \
bash scripts/run_refresh_artifacts_with_exclusions.sh
```

#### Visualization & Viewing

By default, the script generates an **Interactive HTML Dashboard** (`report/index.html`). Due to browser security policies (CORS), you must use a local web server to view it:

```sh
# Start a local web server to view the dashboard
uv run python src/llm_delusions_evals/scripts/serve_report.py
```

#### Static Exports

The script natively outputs high-resolution static plots (PNG, PDF, SVG) for publications to the `report/figures/` directory.

**Note on Anonymity & Exporting:**
Because this report extracts and includes the full conversational history (`history`) that was fed into the evaluated models alongside their generated `excerpt`, **you should NOT export and share reports generated from runs that used a large prior conversational context** (`max_context_messages > 0`). Including extensive historical conversational context increases the risk of inadvertently leaking personally identifiable information (PII) from the original user transcripts. For public dashboards or shared external assets, only export reports generated from "window-only" evaluations (`max_context_messages=0`).

#### Exporting to spiral-web

To copy the generated report bundle into the sibling `spiral-web` site for static embedding:

```sh
uv run python src/llm_delusions_evals/scripts/export_spiral_web_report.py
```

By default this copies from `report/` to:

`../spiral-web/assets/evaluation/report/latest/`

You can override either side:

```sh
uv run python src/llm_delusions_evals/scripts/export_spiral_web_report.py \
  --report-dir report \
  --spiral-web-dir ../spiral-web \
  --target-subdir assets/evaluation/report/preview-2026-04-05 \
  --include-figures
```
